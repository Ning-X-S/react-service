...
async function onlineServer (ctx, next) {
  let cwd = process.cwd()
  let { password = '', name = '', type = 'restart', update_modules = "no_update" } = ctx.params
  if (isPacking) {
  ...
  try {
    process.nextTick(async () => {
      let res = await execSyncNew(`python3 ${config.python}.py ${type} ${update_modules}`)
      isPacking = false
      console.log(res)
    })
    return {
